...
async function main() {
  ...
  if (!['demo', 'prepare', 'build', 'repeat', 'run', 'save-suite', 'evaluate'].includes(command)) throw new Error(`Unknown command: ${command}`);
  ...
  try {
    ...
    if (command === 'evaluate') {
      const draft = await lab.loadSuite(values.input!, values.case);
      ...
      const failed = record.trials.some(t => isAgentFailure(record, t));
      ...
      process.exitCode = incomplete ? 2 : failed ? 1 : 0;
