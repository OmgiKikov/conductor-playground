...
const modes: Record<string, string> = { static: 'Первая реплика', scripted: 'По сценарию', reactive: 'Живой диалог' };
...
const title = (record: Experiment) => {
  const first = plain(record.task).trim().split('\n')[0]?.trim() || 'Проверка агента';
...
<h3>Кодовые проверки</h3>${trial.checks.length ? list(trial.checks.map(c => `${c.passed ? 'Пройдено' : 'Не пройдено'} · ${c.description}: ${c.evidence}`)) : '<p class="muted">Не заданы. Оценки по рубрикам показаны отдельно.</p>'}
...
<article class="metric"><h3>По рубрикам · предварительно</h3><strong>${v.rubric.assessed ? `${v.rubric.passed}<span class="muted"> / ${v.rubric.assessed}</span>` : 'Нет оценок'}</strong><small>${v.rubric.assessed ? escape(modelLabel(record)) : observed.scenarios.some(s => s.metrics?.some(m => m.subject === 'agent')) ? 'Рубрики заданы, оценок пока нет' : 'Рубрики агента не заданы'}${v.rubric.unknown ? ` · ${v.rubric.unknown} неясно` : ''}</small></article>
<article class="metric"><h3>Вердикт на весь диалог</h3><strong>${v.review.reviewed}<span class="muted"> / ${v.review.total}</span></strong><small>Пройдено ${v.review.passed} · не пройдено ${v.review.failed}<br>${v.review.pending} автоматических провалов ждут решения · критерии отдельно</small></article></div>
...
<h3>Следующий шаг</h3>${list(v.nextSteps.map(n => n.text))}</section>
...
<footer><p>Расходы выбранной модели: ${record.usage.costUsd === null ? 'неизвестны' : `$${record.usage.costUsd.toFixed(4)}`} · вызовов ${record.usage.calls}. Расходы внешнего агента и разговора с Pi не входят в эту оценку.</p><p>Локальный автономный отчёт · полные данные и сравнение доступны в JSON-снимке. ${escape(record.id)}</p></footer></main></body></html>`;
}
